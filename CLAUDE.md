...
2. **不做高频小增量重转。** Whisper 内部把 mel 补齐到 30 秒过 encoder，所以 2 秒块和 15 秒块几乎同价（都约 1 秒）。任何"每秒刷新"的设计都会掉队。
   **2026-08-18 增补**：这条约束的是**会落进记录的那条路**。实测复现了它的理由
   （large-v3-turbo：2 秒音频 934ms、25 秒 1486ms，基本与长度无关），也证实了
   它的结论——拿大模型每秒重转会到 70% 占空比，而且更糟的是**会把截断处的猜测
   写进记录**（实测出现过 "Chrissy was weak" 替代 "when democracy was weak"）。
   但**换一个小一个数量级的模型跑一条永不落档的预览层**不在此列：
   whisper-base-q4 同样长度是 83–338ms。听记的灰色预览走这条路，
   见 `backend/preview.py` 与 2026-08-18 的 benchmark。
...
当前在 `v3` 分支。833 个测试通过，无豁免、无 xfail。
...
**P3 边听边出字**（2026-08-18）：屏幕上有两层。**灰色那层**是 80M 小模型每秒
重读一次"正在说的这句"，末尾挂省略号，**不入档、不翻译、不算数**；**定稿那层**
仍是演讲者停顿时由大模型转录完整一句，跟以前一模一样，所以断句质量没有退化。
翻译改为一句一发（带上一句作上下文），实测句子定稿后 1.3–2.4 秒出中文。
实时回放实测：灰字 1.1 秒出现、每 1.1 秒长一次，占空比从 14% 升到约 37%——
**这是这个功能的代价，嫌费电就把 `listen_preview` 关掉**，关掉就回到原来的样子。
