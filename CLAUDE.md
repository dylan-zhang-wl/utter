...
当前在 `v3` 分支。688 个测试通过，无豁免、无 xfail。
...
- **P2a 听写 6/8 完成**：麦克风、热键（左 Option 按住 / 双击左 Control 开关）、埋点计时、
  ...
  **Task 7 应用兼容表**（2026-08-12）：连用一天实测 ChatGPT 68 次、Antigravity IDE
  37 次、微信 8 次、Chrome 2 次，**失败 0 次**；另有 2 次焦点不在目标窗口，
  按铁律 13 缓冲后补注入。这四个可认为已验证，其余仍未测。
